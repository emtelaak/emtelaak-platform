-- Simple seed data for 3 sample offerings
-- Uses correct enum values from actual database schema
...
INSERT INTO offering_financial_projections (
  ...
  assumptionsNotes, createdAt, updatedAt
) VALUES (
  @offering1Id,
  1250, 8500, 750, 185,
  650, 680, 710, 745, 780,
  ...
  'quarterly', 30000000,
  1850, 1250, 650,
  '{"occupancyRate":95,"annualRentGrowth":3,"propertyAppreciation":4,"maintenanceReserve":5,"vacancyRate":5}',
  NOW(), NOW()
);
...
-- Status history for Offering 1
INSERT INTO offering_status_history (
  offeringId, oldStatus, newStatus, changedBy, notes, changedAt
) VALUES (
  @offering1Id, 'draft', 'under_review', 750038, 'Offering submitted for admin review', NOW()
);
...
INSERT INTO offering_financial_projections (
  ...
  'quarterly', 60000000,
  2100, 1450, 800,
  '{"occupancyRate":92,"annualRentGrowth":3.5,"propertyAppreciation":4.5,"maintenanceReserve":6,"vacancyRate":8}',
  NOW(), NOW()
);
...
-- Status history for Offering 2
INSERT INTO offering_status_history (
  offeringId, oldStatus, newStatus, changedBy, notes, changedAt
) VALUES (
  @offering2Id, 'draft', 'under_review', 750038, 'Offering submitted for review', DATE_SUB(NOW(), INTERVAL 5 DAY)
),
(
  @offering2Id, 'under_review', 'approved', 750038, 'All compliance checks passed. Offering approved for funding.', DATE_SUB(NOW(), INTERVAL 2 DAY)
);

-- Approval record for Offering 2
INSERT INTO offering_approvals (
  offeringId, reviewerId, decision, comments, decidedAt, createdAt, updatedAt
) VALUES (
  @offering2Id, 750038, 'approved',
  'Excellent offering structure. Financial projections are realistic and well-documented. All legal documents are in order. Approved for funding.',
...
INSERT INTO offering_financial_projections (
  ...
  'monthly', 18000000,
  1600, 1100, 550,
  '{"occupancyRate":90,"annualRentGrowth":2.5,"propertyAppreciation":3.5,"maintenanceReserve":4,"vacancyRate":10}',
  NOW(), NOW()
);
